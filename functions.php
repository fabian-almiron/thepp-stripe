    ...
    try {
        if ($product_type === 'variation' && $variations) {
            ...
            foreach ($variations as $index => $variation) {
                ...
                // Ensure this block is correctly structured
                if (isset($featured_image_url) && !empty($featured_image_url)) {
                    $variation_stripe_data['images'] = [$featured_image_url]; 
                }
...
function create_checkout_session() {
    ...
    try {
        ...
        $email_for_stripe = null; // Initialize email for Stripe
        $token = null; // Initialize registration token
        ...
        if (is_user_logged_in()) {
            ...
            if ($current_user_obj && $current_user_obj->ID != 0) {
                $email_for_stripe = $current_user_obj->user_email;
                // No new user creation, no registration token needed for existing users
                error_log('[Checkout Session] Logged-in user detected: ' . $email_for_stripe);
            } else {
                // Fallback if user object is not available despite being logged in (should be rare)
                $posted_email_for_loggedin_fallback = isset($_POST['email']) ? sanitize_email(wp_unslash($_POST['email'])) : '';
                if (!empty($posted_email_for_loggedin_fallback) && is_email($posted_email_for_loggedin_fallback)) {
                    $email_for_stripe = $posted_email_for_loggedin_fallback;
                    error_log('[Checkout Session] Logged-in user (fallback), email from POST: ' . $email_for_stripe);
                } else {
                    throw new Exception('User is logged in but email could not be determined and was not provided in POST.');
                }
            }
        } else {
            // Not logged in:
            if ($mode === 'subscription') { // Only create WP user and Stripe customer for subscriptions
                $email_from_form = isset($_POST['email']) ? sanitize_email(wp_unslash($_POST['email'])) : '';
                $username_from_form = isset($_POST['username']) ? sanitize_user(wp_unslash($_POST['username']), true) : '';
                $password_from_form = isset($_POST['password']) ? $_POST['password'] : ''; // Password is not sanitized here; wp_create_user handles hashing

                if (empty($email_from_form) || !is_email($email_from_form) || empty($username_from_form) || empty($password_from_form)) {
                    throw new Exception('Email, username, and password are required for new account creation with a subscription.');
                }
                $email_for_stripe = $email_from_form; // Use this email for Stripe customer
                ...
                error_log('[Checkout Session] New user SUBSCRIPTION. Token: ' . $token . '. Email for Stripe: ' . $email_for_stripe);
            } else {
                // GUEST checkout for non-subscription (simple/variation products)
                // $email_for_stripe remains null. Stripe's checkout page will collect the email.
                // No WordPress user creation, no registration token.
                error_log('[Checkout Session] GUEST checkout for non-subscription. No WP user creation. Stripe will collect email.');
            }
        }
        // --- END OF MODIFIED SECTION ---

        // Set customer_email for Stripe ONLY if $email_for_stripe has been populated.
        // For guest non-subscription checkouts, $email_for_stripe will be null, so customer_email is not pre-filled.
        if ($email_for_stripe) {
            $session_params['customer_email'] = $email_for_stripe;
        } else {
            // If $email_for_stripe is null (guest checkout for non-subscription),
            // explicitly set customer_creation to 'if_required'.
            if ($mode === 'payment') { // Double check it's a payment mode
                $session_params['customer_creation'] = 'if_required';
                error_log('[Checkout Session - create_checkout_session] Guest non-subscription: Setting customer_creation to if_required.');
            }
        }
        ...
        if ($mode === 'payment') {
            ...
            // Conditionally add invoice_creation for payment mode
    ...
    try {
      ...
      //  error_log('Event type: ' . $event->type);
        ...
        if ($event->type === 'checkout.session.completed') {
            ...
            error_log('[Webhook] Processing checkout.session.completed: ' . $session->id);
            
            // --- START ADDED LOGGING ---
            if (isset($session->customer_details)) {
                error_log('[Webhook] Customer Details from session: ' . print_r($session->customer_details, true));
            } else {
                error_log('[Webhook] Customer Details not set in session.');
            }
            if (isset($session->customer_email)) {
                error_log('[Webhook] session->customer_email: ' . $session->customer_email);
            } else {
                error_log('[Webhook] session->customer_email is not set.');
            ...
            $user_id_from_metadata = isset($session->metadata->wordpress_user_id) ? intval($session->metadata->wordpress_user_id) : 0;
            
            // MODIFIED: Use customer_details->email as the primary source for email from the session
            $email_from_session = null;
            if (isset($session->customer_details) && isset($session->customer_details->email) && !empty($session->customer_details->email)) {
                $email_from_session = sanitize_email($session->customer_details->email);
                error_log('[Webhook] Email obtained from session->customer_details->email: ' . $email_from_session);
            } elseif (isset($session->customer_email) && !empty($session->customer_email)) {
                // Fallback to customer_email if customer_details->email is not available (should be rare for completed sessions)
                $email_from_session = sanitize_email($session->customer_email);
                error_log('[Webhook] Email obtained from session->customer_email (fallback): ' . $email_from_session);
            } else {
                error_log('[Webhook] Email could not be obtained from session customer_details or customer_email.');
            }

            $user = null;

            if ($user_id_from_metadata > 0) {
                $user = get_user_by('id', $user_id_from_metadata);
                if ($user) {
                    error_log('[Webhook] User found by wordpress_user_id in metadata: ' . $user->ID);
                } else {
                    error_log('[Webhook] wordpress_user_id (' . $user_id_from_metadata . ') in metadata, but get_user_by(\'id\') failed.');
                }
            }

            if (!$user && $email_from_session) {
                error_log('[Webhook] User not found by metadata ID, trying email: ' . $email_from_session);
                $user = get_user_by('email', $email_from_session);
                if ($user) {
                    error_log('[Webhook] User found by email: ' . $user->ID . ' (' . $email_from_session . ')');
                } else {
                    error_log('[Webhook] User NOT found by email: ' . $email_from_session);
                }
            }


            if (!$user && $registration_data) { // New user registration flow
                error_log('[Webhook] New user registration flow triggered for email: ' . $email_from_session);
                ...
                $found_via = 'unknown';
                if ($user_id_from_metadata > 0) {
                    $check_user_by_id = get_user_by('id', $user_id_from_metadata);
                    if ($check_user_by_id && $check_user_by_id->ID === $user->ID) {
                        $found_via = 'metadata_id';
                    }
                }
                if ($found_via === 'unknown' && $email_from_session) {
                    $check_user_by_email = get_user_by('email', $email_from_session);
                     if ($check_user_by_email && $check_user_by_email->ID === $user->ID) {
                        $found_via = 'email';
                    }
                }
                error_log('[Webhook] Existing user identified. User ID: ' . $user->ID . ', WP Email: ' . $user->user_email . '. Found via: ' . $found_via . '. Stripe Email from session: ' . ($email_from_session ?: 'N/A'));
                // If it's a subscription purchase, update their role and ensure email is verified
                error_log('[Webhook] Checking conditions for existing user subscription. Session Mode: ' . ($session->mode ?? 'Not set') . '. Metadata: ' . print_r($session->metadata, true));
                ...
                if (isset($session->mode) && $session->mode === 'subscription' && isset($session->metadata->contains_subscription) && $session->metadata->contains_subscription === 'true') {
                    error_log('[Webhook] Conditions MET for existing user subscription. Current roles: ' . implode(',', $user->roles));

                    // MOVED UP: Store active subscription ID first for robustness
                    if (isset($session->subscription) && !empty($session->subscription)) {
                        update_user_meta($user->ID, '_active_stripe_subscription_id', $session->subscription);
                        error_log('[Webhook] Stored active Stripe subscription ID: ' . $session->subscription . ' for User ID: ' . $user->ID);
                    } else {
                        error_log('[Webhook] WARNING: Stripe session object did not contain a subscription ID for User ID: ' . $user->ID);
                    }
                    
                    error_log('[Webhook] Attempting to set role to \'subscriber\' for User ID: ' . $user->ID);
                    $user->set_role('subscriber');
                    // Refresh user data to confirm role change for logging
                    $updated_user = new WP_User($user->ID);
                    error_log('[Webhook] Role SET to \'subscriber\' for User ID: ' . $user->ID . '. New roles: ' . implode(',', $updated_user->roles));
                ...
                // NO USER FOUND BY METADATA, EMAIL, AND NO REGISTRATION TOKEN
                // This is where we handle guest checkout for one-time payments if we need to create a user.
                error_log('[Webhook] Checkout session completed. No existing user found by metadata, email, and no valid registration token. Session mode: ' . ($session->mode ?? 'N/A') . '. Email from session: ' . ($email_from_session ?: 'empty'));

                // Check if it's a 'payment' mode (one-time purchase) and we have an email
                if (isset($session->mode) && $session->mode === 'payment' && $email_from_session) {
                    error_log('[Webhook] Guest one-time payment. Attempting to create WP user for: ' . $email_from_session);
                    
                    // Generate a username. Ensure it's unique.
                    $username_base = sanitize_user(explode('@', $email_from_session)[0], true);
                    ...
                    // Create the user
                    $user_id = wp_create_user($username, $password, $email_from_session);

                    if (is_wp_error($user_id)) {
                        error_log('[Webhook] Failed to create user for guest ' . $email_from_session . '. Error: ' . $user_id->get_error_message());
                    } else {
                        error_log('[Webhook] Successfully created new WP user ID: ' . $user_id . ' for guest ' . $email_from_session . ' with username ' . $username);
                        // Set the role to 'customer'
                        $new_user = new WP_User($user_id);
                        $new_user->set_role('customer');
                        error_log('[Webhook] Set role to \'customer\' for user ID: ' . $user_id);
                        
                        // Mark email as verified since payment was successful
                        update_user_meta($user_id, 'email_verified', '1');
                        update_user_meta($user_id, 'registration_date', time());

                        // Send the WordPress new user notification (sends password reset link)
                        wp_new_user_notification($user_id, null, 'user'); // 'user' notification type for new user themselves
                        error_log('[Webhook] Sent new user notification to user ID: ' . $user_id);
                        
                        // Send our custom invoice email as well
                        send_invoice_email($user_id, ['stripe_checkout_session_id' => $session->id]);
                    }
                } else {
                     error_log('[Webhook] Conditions not met for guest user creation (not payment mode or no email). Mode: ' . ($session->mode ?? 'N/A') . '. Email: ' . ($email_from_session ?: 'empty'));
                }
            }
        ...
        if (
            ...
            $subscription_event_object = $event->data->object; // This is a Stripe Subscription object
            $event_subscription_id = $subscription_event_object->id;
            $customer_id = $subscription_event_object->customer;
            // Retrieve the Stripe customer to get the email
            $customer = \Stripe\Customer::retrieve($customer_id); // Consider error handling if customer retrieve fails
            $customer_email = $customer->email;
            error_log('[Webhook] Subscription event (' . $event->type . ') for subscription ID: ' . $event_subscription_id . ', customer: ' . $customer_email);
            if ($customer_email) {
                $user = get_user_by('email', $customer_email);
                if ($user) {
                    $active_stripe_subscription_id_meta = get_user_meta($user->ID, '_active_stripe_subscription_id', true);

                    // Only set to inactive if the CANCELED/DELETED subscription is the one we thought was active,
                    // OR if we have no record of an active subscription for them.
                    if (empty($active_stripe_subscription_id_meta) || $active_stripe_subscription_id_meta === $event_subscription_id) {
                        ...
                        error_log('[Webhook] Attempting to set role to \'subscriber_inactive\' for User ID: ' . $user->ID . ' due to event: ' . $event->type . ' for subscription ID: ' . $event_subscription_id);
                        $user->set_role('subscriber_inactive');
                        // Clear the stored active subscription ID if it matches the one being canceled/deleted
                        if ($active_stripe_subscription_id_meta === $event_subscription_id) {
                            delete_user_meta($user->ID, '_active_stripe_subscription_id');
                            error_log('[Webhook] Cleared _active_stripe_subscription_id for User ID: ' . $user->ID);
                        }
                        $updated_user_for_inactive = new WP_User($user->ID);
                        error_log('[Webhook] Role SET to \'subscriber_inactive\' for User ID: ' . $user->ID . '. New roles: ' . implode(',', $updated_user_for_inactive->roles));
                    } else {
                        error_log('[Webhook] IGNORED subscription event (' . $event->type . ') for subscription ID: ' . $event_subscription_id . ' for User ID: ' . $user->ID . '. Does not match active ID: ' . $active_stripe_subscription_id_meta);
                    }
                } else {
                    error_log('[Webhook] No WordPress user found for email: ' . ($customer_email ?: 'empty') . ' for subscription event: ' . $event->type);
                }
            }
        }
...
// ADDED: Ensure 'customer' role exists
function ensure_customer_role_exists() {
    if (!get_role('customer')) {
        add_role('customer', 
                 'Customer',
                 array(
                     'read' => true, // True allows this user to read posts
                 )
        );
        error_log('Created \'customer\' role.');
    } else {
        // Optional: Log if role already exists for debugging, or remove this else block
        // error_log('\'customer\' role already exists.');
    }
}
add_action('init', 'ensure_customer_role_exists');

/// handle user access conrol

function handle_user_access_control() {
    // Get current page URL at the very top of the function
    $current_url = $_SERVER['REQUEST_URI'];

    // If current page is /my-account/ or any of its subpages, this function should not interfere.
    // For logged-in users, they can access it.
    // For non-logged-in users, other mechanisms (WP default, WooCommerce, etc.) would handle login prompts.
    if (strpos($current_url, '/my-account/') === 0) {
        return;
    }

    // Assuming the user has removed the block comments /* and */
    // from around the main body of this function as previously indicated.

    error_log('handle_user_access_control function called for general URL: ' . $current_url);
    ...
    if (is_user_logged_in()) {
        ...
        // $current_url is already defined at the top of the function.
        // Original log for current URL can be kept or removed if deemed redundant.
        // error_log('Current URL: ' . $current_url); // This line (original 1477) can be removed or kept for specific debugging.
        ...
        if (in_array('pending', $user->roles) && in_array('subscriber', $user->roles) && $email_verified !== '1') {
            ...
            if (!in_array('subscriber', $user->roles) && !in_array('administrator', $user->roles)) {
                ...
                if ($redirect) {
                    error_log('Redirecting logged-in non-subscriber/non-admin to /my-account/ for restricted content access.');
                    wp_redirect(home_url('/my-account/'));
                    exit;
                }
            }
        ...
        // $current_url is already defined at the top of the function.
        // Original log for current URL can be kept or removed if deemed redundant.
        // error_log('Current URL (not logged in): ' . $current_url); // This line (original 1529) can be removed or kept.

        foreach ($restricted_pages as $page) {
            if (strpos($current_url, $page) === 0) {
                error_log('Redirecting non-logged-in user from ' . $current_url . ' to /my-account/.');
                wp_redirect(home_url('/my-account/'));
                exit;
            }
        }
...
// Update send_invoice_email to BCC ninthsouthdigital@gmail.com on every order
function send_invoice_email($user_id, $order_data = []) {
    ...
    error_log('[Send Invoice Email] Attempting to send invoice email to customer: ' . $to . ' for order/session: ' . $template_vars['order_id']);
    if (wp_mail($to, $subject, $message, $headers)) {
        error_log('[Send Invoice Email] Email sent successfully to customer: ' . $to . ' for order/session: ' . $template_vars['order_id']);
    } else {
        error_log('[Send Invoice Email] Failed to send email to customer: ' . $to . ' for order/session: ' . $template_vars['order_id']);
    }

    // Send a second email directly to ninthsouthdigital@gmail.com
    // $admin_email = 'ninthsouthdigital@gmail.com';
    // error_log('[Send Invoice Email] Attempting to send duplicate invoice email to admin: ' . $admin_email . ' for order/session: ' . $template_vars['order_id']);
    // if (wp_mail($admin_email, $subject, $message, $headers)) {
    //     error_log('[Send Invoice Email] Email sent successfully to admin: ' . $admin_email . ' for order/session: ' . $template_vars['order_id']);
    // } else {
    //     error_log('[Send Invoice Email] Failed to send email to admin: ' . $admin_email . ' for order/session: ' . $template_vars['order_id']);
    // }
}
...
function tpp_custom_login_redirect( $redirect_to, $request, $user ) {
    ...
    if ( isset( $user->roles ) && is_array( $user->roles ) ) {
        ...
        } elseif ( in_array( 'subscriber_inactive', $user->roles ) ) {
            // Inactive subscribers go to the My Account page
            return home_url('/my-account/');
        } else {
            // Other roles (like pending, etc.) go to the My Account page as a sensible default
            return home_url('/my-account/');
        }
    }
...
function handle_cart_to_stripe_checkout() {
    ...
    $original_session_cart = $_SESSION['cart'] ?? [];

    if (empty($original_session_cart)) {
        wp_send_json_error(['message' => 'Your cart is empty.']);
        return;
    }

    $cart_for_stripe_payment_session = [];
    $has_non_subscription_items = false;
    $has_subscription_items = false;
    $subscriptions_to_redirect = [];


    foreach ($original_session_cart as $item_key => $item) {
        if (isset($item['product_type'])) {
            if ($item['product_type'] === 'subscription') {
                $has_subscription_items = true;
                $subscriptions_to_redirect[$item_key] = $item; // Keep track of subscriptions
            } else {
                $cart_for_stripe_payment_session[$item_key] = $item;
                $has_non_subscription_items = true;
            }
        }
    }

    if ($has_non_subscription_items) {
        // User is checking out at least one simple/variation product.
        // Remove all subscriptions from the current session cart if they were mixed.
        if ($has_subscription_items) {
            $_SESSION['cart'] = $cart_for_stripe_payment_session; // Update session to only contain non-subscription items
            // Optionally: Add a WordPress notice here that subscriptions were removed and need separate purchase.
            // e.g., if (function_exists('wc_add_notice')) { wc_add_notice('Subscription products have been removed from this order as they need to be purchased separately. Your non-subscription items remain.', 'notice'); }
            error_log('[Cart Checkout] Mixed cart: Subscriptions removed from session. Proceeding with non-subscription items.');
        }
        // $cart_for_stripe_payment_session already contains only non-subscription items.
        // If $cart_for_stripe_payment_session is empty now, it means the cart *only* had subscriptions that were just removed,
        // or it was a mix and somehow only subscriptions were left after an error (which is less likely).
        // The initial check for empty $original_session_cart and the check below should cover this.

    } else if ($has_subscription_items) {
        // Cart ONLY contains subscription items. This flow is not for mode:subscription.
        // Redirect to the dedicated checkout flow for the first subscription.
        $first_sub_item_key = array_key_first($subscriptions_to_redirect);
        $first_sub_item = $subscriptions_to_redirect[$first_sub_item_key];
        
        if ($first_sub_item && isset($first_sub_item['product_id'])) {
            // Clear the cart as subscriptions are handled by direct checkout links
            // $_SESSION['cart'] = []; // Or just keep the subscription for the redirect
            // $_SESSION['cart'][$first_sub_item_key] = $first_sub_item;

            $redirect_url = home_url('/checkout/?checkout_product_id=' . $first_sub_item['product_id'] . '&quantity=1');
            error_log('[Cart Checkout] Cart contains only subscriptions. Redirecting to: ' . $redirect_url);
            wp_send_json_success(['url' => $redirect_url, 'message' => 'Redirecting to subscription checkout.']);
        } else {
            wp_send_json_error(['message' => 'Error processing subscription from cart. Invalid item data.']);
        }
        return; // Stop further processing in this function
    } else {
        // Cart was empty or contained no processable items.
        wp_send_json_error(['message' => 'Your cart contains no items eligible for checkout.']);
        return;
    }
    
    // After filtering, if $cart_for_stripe_payment_session is empty, it means no non-subscription items are left to checkout.
    if (empty($cart_for_stripe_payment_session)) {
        // This could happen if original cart was only subscriptions (handled above by redirect)
        // or if it was mixed, subscriptions were removed, and for some reason no non-subscriptions were actually there.
        wp_send_json_error(['message' => 'No non-subscription items to checkout. Subscriptions must be purchased separately.']);
        return;
    }

    // Proceed with $cart_for_stripe_payment_session for a 'payment' mode Stripe session
    try {
        ...
        foreach ($cart_for_stripe_payment_session as $item_key => $item) { // Use $cart_for_stripe_payment_session
            ...
            if (isset($item['product_type']) && ($item['product_type'] === 'single' || $item['product_type'] === 'variation')) {
                $needs_shipping = true;
            }
        }
        ...
        $session_params = [
            ...
            // Set invoice_creation to enabled for all checkouts from the cart page.
            'invoice_creation'     => ['enabled' => true],
        ];

        // 8. Customer Email (if logged in) & Customer Creation for Guests
        if (is_user_logged_in()) {
            $current_user = wp_get_current_user();
            if ($current_user && !empty($current_user->user_email)) {
                $session_params['customer_email'] = $current_user->user_email;
            }
            // For logged-in users, invoice_creation is already enabled globally for the session_params above.
            error_log('[Cart Checkout] Logged-in user: Invoice creation is enabled.');
        } else {
            // For guest checkouts (non-logged-in users) for one-time payments,
            // explicitly set customer_creation to 'if_required'.
            $session_params['customer_creation'] = 'if_required';
            // For guest checkouts, ensure invoice_creation remains enabled (it's set globally above).
            error_log('[Cart Checkout] Guest checkout: Setting customer_creation to if_required. Invoice creation is enabled.');
        }
        ...
        $product_names = array_map(function($item) { 
            return isset($item['name']) ? $item['name'] : 'Unknown Product'; 
        }, $cart_for_stripe_payment_session); // Use $cart_for_stripe_payment_session here
...
add_filter('wp_nav_menu_items', 'replace_logout_placeholder_link', 10, 2);

function replace_logout_placeholder_link($items, $args) {
    if (is_user_logged_in()) {
        $logout_url = wp_logout_url(home_url());
        $items = str_replace('#logout', esc_url($logout_url), $items);
    }
    return $items;
}


// --- Admin Test Button for send_invoice_email Debugging ---
add_action('admin_menu', function() {
    add_menu_page('Test Invoice Email', 'Test Invoice Email', 'manage_options', 'test-invoice-email', 'test_invoice_email_page');
});

function test_invoice_email_page() {
    ...
    if (isset($_POST['send_test_invoice'])) {
        ...
        if ($current_user && $current_user->ID) {
            // Dummy order data
            $order_data = [
                'stripe_checkout_session_id' => null, // No real session, will use fallback
                'items' => '<tr><td>Test Product</td><td>1</td><td>$10.00</td><td>$10.00</td></tr>',
                'total' => '$10.00',
                'shipping' => '123 Test St, Test City',
                'billing' => '123 Test St, Test City',
                'date' => date('Y-m-d'),
                'id' => 'TEST123',
            ];
            send_invoice_email($current_user->ID, $order_data);
            echo '<div class="notice notice-success"><p>Test invoice email triggered. Check your logs and email inboxes.</p></div>';
        } else {
            echo '<div class="notice notice-error"><p>No valid user found.</p></div>';
        }
    }
    echo '<form method="post"><button type="submit" name="send_test_invoice" class="button button-primary">Send Test Invoice Email</button></form>';
}
?>